...
// --- Reusable Space Card Component (No changes here) ---
const SpaceCard = ({ space, onSelect }) => (
  <div onClick={() => onSelect(space)} className="bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 transform hover:-translate-y-1 cursor-pointer flex flex-col">
    <div className="relative">
      ...
      <div className="absolute top-3 left-3 bg-[#0052cc] text-white text-xs font-bold py-1.5 px-3 rounded-full flex items-center">
        <VerifiedIcon />Vetted
      </div>
    </div>
    <div className="p-5 flex-grow flex flex-col">
      ...
      <div className="flex items-center mt-3 text-gray-800">
        <StarIcon className="h-5 w-5 text-yellow-500" />
        <span className="font-bold ml-1">{space.rating}</span>
        <span className="text-gray-500 ml-2">({space.reviews} reviews)</span>
      </div>
      <div className="flex items-center space-x-4 mt-4 border-t pt-4">
        <div className="flex items-center space-x-2 text-sm text-gray-600"><WifiIcon /> <span>Fast WiFi</span></div>
        <div className="flex items-center space-x-2 text-sm text-gray-600"><PowerIcon /> <span>24/7 Power</span></div>
      </div>
      <div className="mt-auto pt-4 text-right">
        <p className="text-2xl font-bold text-[#0052cc]">₦{space.price.toLocaleString()}<span className="text-base font-normal text-gray-500">/{space.priceType}</span></p>
      </div>
    </div>
  </div>
);
...
export default function SearchResultsPage({ spaces, onSelectSpace, isLoading, onBack }) {
  ...
  // --- NEW --- State for managing the sort order
  ...
  }, [spaces, priceFilter, amenitiesFilter, sortOrder]); // sortOrder is now a dependency
    ...
    <div className="bg-white">
      <div className="container mx-auto px-6 py-8">
        <div className="flex justify-between items-center mb-6">
            <button onClick={onBack} className="inline-flex items-center text-gray-600 hover:text-[#0052cc] font-semibold">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5 mr-2"><path d="M19 12H5"/><path d="m12 19-7-7 7-7"/></svg>
                Back to Home
            </button>
            <h1 className="text-2xl font-bold text-gray-800 hidden sm:block">
                Workspaces in <span className="text-[#0052cc]">Ibadan</span>
            </h1>
        </div>

        <div className="flex flex-col lg:flex-row gap-8">
          {/* --- Filters Sidebar (Left) - (No changes here) --- */}
              ...
              <div className="bg-gray-50 p-6 rounded-lg border">
                ...
                <div>
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-[#0052cc]">₦{priceFilter.toLocaleString()}</span>
                  </div>
          ...
          <main className="w-full lg:w-3/4">
            ...
            <div className="flex justify-between items-center mb-4 pb-4 border-b">
                <p className="text-gray-600">
                    Showing <span className="font-bold">{processedSpaces.length}</span> spaces
                </p>
                <div>
                    <select 
                        value={sortOrder}
            ...
            {isLoading ? ( <p>Loading...</p> ) : (
              // --- UPDATED --- Use processedSpaces and show a message if it's empty
              <>
                {processedSpaces.length > 0 ? (
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {processedSpaces.map((space) => (
                      <SpaceCard key={space.id} space={space} onSelect={onSelectSpace} />
                    ))}
                  </div>
                ) : (
                  <div className="text-center py-20">
                    <h3 className="text-xl font-semibold">No results found</h3>
                    <p className="text-gray-500 mt-2">Try adjusting your filters to find more spaces.</p>
                  </div>
                )}
              </>
            )}
