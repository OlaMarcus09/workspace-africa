...
// --- COMPONENTS ---
const Header = () => ( <header className="bg-white shadow-sm sticky top-0 z-50"><div className="container mx-auto px-6 py-4 flex justify-between items-center"><img src="https://res.cloudinary.com/dmqjicpcc/image/upload/v1760286253/WorkSpaceAfrica_bgyjhe.png" alt="WorkSpace Africa Logo" className="h-8 w-auto" /><nav className="hidden md:flex items-center space-x-8"><a href="#" className="text-gray-600 hover:text-[#0052cc]">Find a Space</a><a href="#" className="text-gray-600 hover:text-[#0052cc]">List Your Space</a></nav><div className="flex items-center space-x-4"><button className="hidden md:block text-gray-600 hover:text-[#0052cc]">Sign In</button><button className="bg-[#FFB400] text-white font-semibold py-2 px-5 rounded-lg hover:bg-opacity-90 transition-all">Sign Up</button></div></div></header> );
...
export default function App() {
  ...
  // This effect runs once when the component mounts to fetch data from Firebase
  useEffect(() => {
  ...
  const handleSelectSpace = (space) => { setSelectedSpace(space); window.scrollTo(0, 0); };
  const handleGoBack = () => { setSelectedSpace(null); };

  if (selectedSpace) {
    return ( <> <Header /> <SpaceDetailPage space={selectedSpace} onBack={handleGoBack} /> <Footer /> </> );
  }
  ...
  return (
    <div className="bg-gray-50 font-sans">
      ...
      <main>
        <HeroSection />
        <FeaturedSpaces spaces={spaces} onSelect={handleSelectSpace} isLoading={isLoading} />
      </main>
      <Footer />
    </div>
  );
