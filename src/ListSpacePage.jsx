...
// --- A new icon for the upload area ---
...
export default function ListSpacePage({ setView, currentUser }) {
  // --- UPDATED --- State to hold all form data with the new pricing structure
  ...
  // --- NEW --- Handler for the nested pricing state
  ...
  const handleSubmit = (e) => {
    ...
    // A partner must provide at least one price
    if (!spaceDetails.pricing.day && !spaceDetails.pricing.week && !spaceDetails.pricing.month) {
        alert("Please enter at least one price for your space (daily, weekly, or monthly).");
        return;
    }
    setIsLoading(true);
    // --- In the NEXT step, we will add Firebase logic for form AND image upload ---
    console.log("Submitting space details:", {
      ...spaceDetails,
      ownerId: currentUser.uid
    });
    console.log("Image to upload:", mainImage);
    alert("Form submitted! (Next, we'll connect this to Firebase Storage and Firestore).");
    setIsLoading(false);
  };
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-8">
            ...
            <div className="border-b pb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                    <div>
                        ...
                        <input type="text" name="name" id="name" required value={spaceDetails.name} onChange={handleInputChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#0052cc] focus:ring-[#0052cc] sm:text-sm p-2"/>
                    </div>
                    <div>
                        <label htmlFor="location" className="block text-sm font-medium text-gray-700">Location / Area</label>
                        <input type="text" name="location" id="location" required placeholder="e.g., Bodija, Ibadan" value={spaceDetails.location} onChange={handleInputChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#0052cc] focus:ring-[#0052cc] sm:text-sm p-2"/>
                    </div>
                </div>
                <div className="mt-6">
                    <label htmlFor="description" className="block text-sm font-medium text-gray-700">Description</label>
                    <textarea name="description" id="description" rows="4" required value={spaceDetails.description} onChange={handleInputChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#0052cc] focus:ring-[#0052cc] sm:text-sm p-2"></textarea>
                </div>
            </div>
                ...
                <div className="mt-4">
                    <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                        <div className="space-y-1 text-center">
                            {imagePreview ? (
                                <img src={imagePreview} alt="Space preview" className="mx-auto h-48 w-auto rounded-md"/>
                            ) : (
                                <>
                                    <UploadCloudIcon />
                                    <div className="flex text-sm text-gray-600">
                                        <label htmlFor="file-upload" className="relative cursor-pointer bg-white rounded-md font-medium text-[#0052cc] hover:text-blue-700 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                    ...
                    {imagePreview && (
                        <button type="button" onClick={() => {setImagePreview(''); setMainImage(null);}} className="mt-2 text-sm text-red-600 hover:text-red-800">
                            Remove image
                        </button>
                    )}
                </div>
            ...
            <div className="border-b pb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
                    <div>
                        <label htmlFor="day" className="block text-sm font-medium text-gray-700">Daily Rate (₦)</label>
                        <input type="number" name="day" id="day" min="1000" step="500" value={spaceDetails.pricing.day} onChange={handlePricingChange} placeholder="e.g., 5000" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#0052cc] focus:ring-[#0052cc] sm:text-sm p-2"/>
                    </div>
                    <div>
                        <label htmlFor="week" className="block text-sm font-medium text-gray-700">Weekly Rate (₦)</label>
                        <input type="number" name="week" id="week" min="5000" step="1000" value={spaceDetails.pricing.week} onChange={handlePricingChange} placeholder="e.g., 25000" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#0052cc] focus:ring-[#0052cc] sm:text-sm p-2"/>
                    </div>
                     <div>
                        <label htmlFor="month" className="block text-sm font-medium text-gray-700">Monthly Rate (₦)</label>
                        <input type="number" name="month" id="month" min="20000" step="5000" value={spaceDetails.pricing.month} onChange={handlePricingChange} placeholder="e.g., 100000" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#0052cc] focus:ring-[#0052cc] sm:text-sm p-2"/>
                    </div>
                </div>
            </div>
            ...
            <div className="pt-6 text-right">
                <button type="submit" disabled={isLoading} className="bg-[#FFB400] text-white font-bold py-3 px-8 rounded-lg hover:bg-opacity-90 transition-all disabled:bg-gray-400">
